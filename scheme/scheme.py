...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(func)
        if isinstance(func, MacroProcedure): # check if this func is Macro
            return scheme_eval(func.apply_macro(rest, env), env) # eval the applied macro
        else:
            return scheme_apply(func, rest.map(lambda exp: scheme_eval(exp, env)), env)
        # END PROBLEM 4
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        return self.env.make_child_frame(self.formals, args) # self.env for lexically scoped
                                                             # env for dynamically scoped
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        return func_name
        # target       => (f x y)
        # expressions  => ((f x y) (+ x y))
        # removed name => ((x y) (+ x y))
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    # bindings = ((x 5)) 
    # bindings = ((x 2)(y x)) we need to handle this by working on each list of the nested list
    while bindings is not nil:
        validate_form(bindings.first, 2, 2) # a list can only have min 2 and max 2, because that's what we do with let
        names = Pair(bindings.first.first, names)
        values = Pair(scheme_eval(bindings.first.rest.first, env), values) # to test whether or not a variable was defined 
        bindings = bindings.rest # work on other lists of bindings
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        macro = MacroProcedure(target.rest, expressions.rest, env) # create a macro procedure
        env.define(target.first, macro) # bind the macro proc to the given name
        return target.first # return that name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
    # END Problem EC 2
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a dynamically-scoped call evaluated in Frame ENV, the environment."""
        return env.make_child_frame(self.formals, args) # self.env for lexically scoped
                                                        # env for dynamically scoped
    # END PROBLEM 15
...
#            if scheme_symbolp(result.expr.first) and result.expr.first in SPECIAL_FORMS:
#                return SPECIAL_FORMS[result.expr.first](result.expr.rest, env)
#            else:
#                func = optimized_eval(result.expr, result.env)
#                validate_procedure(function)
#                result = complete_apply(func, result.expr.rest, result.env)
        return result
        # END PROBLEM EC 1
